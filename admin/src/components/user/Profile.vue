<template>
  <a-card>
    <a-form-model
      ...
      :wrapper-col="{ span: 12 }"
    >
      <a-form-model-item label="author name">
        <a-input style="width: 300px" v-model="profileInfo.name"></a-input>
      </a-form-model-item>

      <a-form-model-item label="Infor">
        <a-input type="textarea" v-model="profileInfo.desc"></a-input>
      </a-form-model-item>

      <a-form-model-item label="weburl">
        <a-input
          style="width: 300px"
          v-model="profileInfo.icp_record"
        ></a-input>
      </a-form-model-item>

      <a-form-model-item label="QQ number">
      ...
      <a-form-model-item label="bilibili">
      ...
      <a-form-model-item label="avatar image">
        <a-upload
          listType="picture"
          name="file"
          :action="upUrl"
  ...
  methods: {
    ...
    async getProfileInfo () {
      const { data: res } = await this.$http.get(
        `admin/profile/${this.profileInfo.id}`
      )
      if (res.status !== 200) {
    ...
    avatarChange (info) {
      ...
      if (info.file.status === 'done') {
        this.$message.success('upload images success')
        const imgUrl = info.file.response.url
        this.profileInfo.avatar = imgUrl
      } else if (info.file.status === 'error') {
        this.$message.error('upload images failed')
      }
    },
    // 上传头像背景图
    imgChange (info) {
      ...
      if (info.file.status === 'done') {
        this.$message.success('Upload image success')
        const imgUrl = info.file.response.url
    ...
    async updateProfile () {
      ...
      this.$message.success('Update personal information success')
